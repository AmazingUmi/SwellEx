"""Training loop and metric helpers."""
...
from .data import fixed_split_bundle, random_split_bundle, resolve_h5_paths
from .model import (
    build_model,
    build_model_config,
...
def run_epoch(
    ...
    for x, y_km in loader:
        ...
        batch_size = x.size(0)
        pred_km = pred_norm.detach() * y_std + y_mean
        err = pred_km - y_km
...
def train(args: argparse.Namespace) -> None:
    ...
    if resume_checkpoint is None:
        ...
        if config.input_elements != elements or config.input_freq_bins != freq_bins:
            raise ValueError(
                "Resume checkpoint input shape does not match current data: "
                f"checkpoint has {config.input_elements} x {config.input_freq_bins}, "
                f"data has {elements} x {freq_bins}."
            )
    ...
    print(f"Input: 2 x {elements} x {freq_bins}")
    ...
    for epoch in range(start_epoch, end_epoch + 1):
        train_metrics = run_epoch(
            model, train_loader, criterion, device, y_mean, y_std, optimizer
        )
        val_metrics = run_epoch(model, val_loader, criterion, device, y_mean, y_std)
        scheduler.step()
